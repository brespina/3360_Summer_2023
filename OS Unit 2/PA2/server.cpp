...
TODO: Add concise comments to main.
*/
...
void handleRequest(int sock) {
    ...
    check(sock, 0, "ERROR on accept");  //validate socket acceptance

    // read in size of input str from client. create temp char array buffer. read in input str.
    int size;
    check(read(sock, &size, sizeof(int)), 0, "ERROR reading from socket");  
    char *buffer = new char[size+1];
    bzero(buffer, size+1);
    check(read(sock, buffer, size), 0, "ERROR reading from socket");  
